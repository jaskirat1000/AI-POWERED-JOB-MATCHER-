...
OPENAI_MODEL = os.getenv('OPENAI_MODEL', "azure/gpt-4.1")  # Use Azure model format for LiteLLM
OPENAI_FAST_MODEL = os.getenv('OPENAI_FAST_MODEL', "azure/gpt-4.1")  # Use Azure model format for LiteLLM
...
def create_clients():
    ...
    try:
        ...
        if claude_key and claude_key != "your-anthropic-api-key-here":
            clients["anthropic"] = anthropic.Anthropic(api_key=claude_key)
    except Exception as e:
        print(f"Warning: Could not initialize Anthropic client: {e}")
    
    # Try to create OpenAI client
    try:
        openai_key = os.getenv('OPENAI_API_KEY') or os.getenv('LITELLM_API_KEY')
        base_url = os.getenv('LITELLM_BASE_URL')
        print(f"Debug: API Key found: {openai_key[:10]}..." if openai_key else "Debug: No API key found")
        print(f"Debug: Base URL: {base_url}")
        
        if openai_key and openai_key != "your-openai-api-key-here":
            if base_url:
                # Use LiteLLM endpoint
                print(f"Debug: Using LiteLLM endpoint: {base_url}")
                ...
                # Use standard OpenAI
                print("Debug: Using standard OpenAI endpoint")
...
default_anthropic_client = clients.get("anthropic")
default_openai_client = clients.get("openai")
...
def talk_to_openai(prompt,
    ...
    try:
        print(f"Making API call to OpenAI with model: {OPENAI_MODEL}")
...
def extract_text_and_image_from_pdf(file_path):
    try:
        ...
        # Try to extract images if dependencies are available
        try:
            ...
            print(f"Debug: Extracted {len(images)} images from PDF")
            
        except ImportError as e:
            print(f"Debug: Image extraction dependencies not available: {e}")
            print("Debug: Continuing with text-only extraction")
        except Exception as e:
            print(f"Debug: Error during image extraction: {e}")
            print("Debug: Continuing with text-only extraction")

        if not text.strip():
            print(f"Debug: No text extracted from PDF {file_path}")
            return "", []

        return text.strip(), resume_images

    except Exception as e:
        print(f"Debug: Error extracting text and image from PDF {file_path}: {str(e)}")
...
def extract_job_requirements(job_desc, client=None):
    ...
    if not response or response.strip() == "":
        print("Error: Received empty response from AI. Please check your API key and try again.")
        return None
    ...
    try:
        ...
        print(f"Error: Could not parse AI response. Raw response: {response[:200]}...")
        return None
    except Exception as e:
        logging.error(f"Unexpected error parsing job requirements: {str(e)}")
        print(f"Error: Unexpected error parsing AI response: {str(e)}")
        return None
...
You can only speak in clean, concise, Markdown format.     
    ...
    if generate_pdf:
        ...
        if generate_pdf:
            try:
                # Try to import WeasyPrint dynamically
                from weasyprint import HTML
                HTML(string=html_with_style).write_pdf(pdf_filename)
                logging.info(f"PDF file created: {pdf_filename}")
            except ImportError:
                logging.warning("PDF generation requested but WeasyPrint is not available")
                print("Warning: PDF generation is disabled because WeasyPrint is not properly installed.")
                print("The script will continue with resume matching without PDF generation.")
            except Exception as e:
